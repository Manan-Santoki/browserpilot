...
/** React Native counterpart of the desktop raw Gemini Live transport. */
export class GeminiLiveClient {
  ...
  private async connect(): Promise<void> {
    ...
    const provisioned = canResume ? this.resumableToken! : await this.tokenProvider();
    this.resumableToken = provisioned;
    this.callbacks.onDiagnostic?.(canResume ? "token.reused" : "token.provisioned");
    if (!this.desiredOpen || generation !== this.generation) return;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      socket.onmessage = (event) => {
        void decodeServerMessage(event.data).then((message) => {
          ...
          if (message.goAway && this.desiredOpen) this.scheduleReconnect(true);
        }).catch((error) => {
      ...
      socket.onclose = (event) => {
        ...
        if (this.desiredOpen && generation === this.generation) this.scheduleReconnect(false);
      };
    });
  }
